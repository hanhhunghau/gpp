...
class GenerateLayout {
  ...
  function synchronize() {
    $text=$this->source;
    $text=strtr($text,array('MAP[\''=>'$this->map[\'','%5B%5B|'=>'[[|','|%5D%5D'=>'|]]','%5B%5B%7C'=>'[[|','%7C%5D%5D'=>'|]]'));
    ...
    return preg_replace(array('/<textarea name="([^\[\"]*)\[\[\|([^\|]+)\|\]\]"([^\>]+)>([^\[\"]*)\[\[\|([^\|]+)\|\]\]<\/textarea>/i','/<textarea name="([^\"]+)"([^\>]+)>([^\[\"]*)\[\[\|([^\|]+)\|\]\]<\/textarea>/i','/<textarea name="([^\[\"]*)\[\[\|([^\|]+)\|\]\]"([^\>]+)>([^\<]*)<\/textarea>/i','/<textarea name="([^\"]+)"([^\>]+)>([^\<]*)<\/textarea>/i','/<input name="([^\[\"]*)\[\[\|([^\|]+)\|\]\]" type="([^\"]+)" value="\[\[\|([^\|]+)\|\]\]"([^\>]*)>/i','/<input name="([^\"]+)" type="([^\"]+)" id="([^\"])+" value="([^\[\"]*)\[\[\|([^\|]+)\|\]\]">/i','/<input name="([^\"]+)" type="([^\"]+)" id="([^\"])+" value="([^\"]*)">/i','/<input name="([^\[\"]*)\[\[\|([^\|]+)\|\]\]" type="([^\"]+)"([^\>]+)>/i','/<input name="([^\"\|\[]+)" type="([^\"]+)"([^\>\|\[]+)>/i','/\[\[\.([^\.\]]+)\.\]\]/'),array('<textarea  name="\\1[[|\\2|]]"\\3><?php echo String::html_normalize(URL::get(\'\\1\'.[[=\\2=]],\'\\4\'.[[=\\5=]]));?></textarea>','<textarea  name="\\1"\\2><?php echo String::html_normalize(URL::get(\'\\1\',\'\\3\'.[[=\\4=]]));?></textarea>','<textarea  name="\\1[[|\\2|]]"\\3><?php echo String::html_normalize(URL::get(\'\\1\'.[[=\\2=]],\'\\4\'));?></textarea>','<textarea  name="\\1"\\2><?php echo String::html_normalize(URL::get(\'\\1\',\'\\3\'));?></textarea>','<input  name="\\1[[|\\2|]]" value="<?php echo String::html_normalize(URL::get(\'\\1\'.[[=\\2=]],[[=\\4=]]));?>"\\5 type ="\\3">','<input  name="\\1" type ="\\2" id="\\3" value="<?php echo String::html_normalize(URL::get(\'\\1\',\'\\4\'.[[=\\5=]]));?>">','<input  name="\\1" type ="\\2" id="\\3" value="<?php echo String::html_normalize(URL::get(\'\\1\',\'\\4\'));?>">','<input  name="\\1[[|\\2|]]"\\4 type ="\\3" value="<?php echo String::html_normalize(URL::get(\'\\1\'.[[=\\2=]]));?>">','<input  name="\\1"\\3 type ="\\2" value="<?php echo String::html_normalize(URL::get(\'\\1\'));?>">','<?php echo PageSetting::language(\'\\1\');?>'),$text);
  }
  ...
  function generate_text($text) {
    ...
    $text=str_replace('</form>','<input type="hidden" name="form_block_id" value="<?php echo isset(Module::$current->data)?Module::$current->data[\'id\']:\'\';?>"> 
            </form > 
            '.$make_onsubmit.' 
            ',$text);
    $result='';
      ...
      if($pos2=strpos($text,'"',$pos+15)) {
        ...
        if($items=$this->get_text_variable($var.'_list')) {
          ...
          $result.='<?php 
                    ...
                    ?><script>$(\''.$var.'\').value = "<?php echo addslashes(URL::get(\''.$var.'\',isset($this->map[\''.$var.'\'])?$this->map[\''.$var.'\']:\'\'));?>";</script> 
    ';
          $p2=strpos($text,'</select>',$p+1);
          $result.=preg_replace(array('/<option value="([^\[\"]*)\[\[\|([^\|]+)\|\]\]">/i','/<option value="([^\"]+)">/i'),array('<option value="\\1[[|\\2|]]" <?php  
